...
function MainCatalog() {
    const [titleSearch, setTitleSearch] = useState(""); // Состояние для строки поиска
    const [titleList, setTitleList] = useState(Product); // Список продуктов

    useEffect(() => {
        const debounce = setTimeout(() => {
            const filteredProducts = findProduct(titleSearch, Product);
            setTitleList(filteredProducts); // Обновляем список продуктов
        }, 300);

        return () => clearTimeout(debounce); // Очищаем таймаут
    }, [titleSearch]);

    return (
        <div className="w-full  min-h-[1000px] flex flex-col items-center justify-center">
            <div className="w-[90%] h-[70px] shadow-xl mt-32 rounded-lg flex justify-center items-center">
                <input
                    type="text"
                    placeholder="Введите название товара..."
                    value={titleSearch} // Связываем с состоянием
